...
export default function DropDownColorPalette({ setColor, color }) {
  return (
    <Menu as="div" className="relative inline-block text-left">
      <div>
        <Menu.Button className={` inline-flex w-full justify-between rounded-md border-gray-300 bg-slate-100 px-3 py-4  text-gray-700  focus:outline-none focus:ring-2 focus:ring-violet-500 focus:ring-offset-2 focus:ring-offset-gray-100`}>
            ...
            {initialColorPalette.map((color) => (
                <Menu.Item key={color.code}>               
                  <div
                   ...
                   className={`mx-0.5 w-7 h-7 ${color.code} rounded-full border-2 shadow hover:border-slate-800 cursor-pointer`}></div>            
                </Menu.Item>
            ))}    
          </div>
        </Menu.Items>
